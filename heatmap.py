...
#    print(datadifference)
    ...
    data0 = np.loadtxt(r"20180506/heatmap/01_3DMetal_2D0.csv", delimiter=",", skiprows=2, usecols=range(71,321))
    data1 = np.loadtxt(r"20180506/heatmap/02_3DMetal_3D255.csv", delimiter=",", skiprows=2, usecols=range(71,321))
    datadifference = data1 - data0
    fig, ax = plt.subplots(3, sharex=True)
#    fig, ax = plt.subplots(1, 3)
    im0=ax[0].imshow(data0, extent=extent, aspect=calculate_aspect(shape, extent), cmap='afmhot')
    fig.colorbar(im0, ax = ax[0])
    ax[0].set_title('3DLED On Metal, Thermal Eng Off, 2D:0')
    ...
    ax[1].set_title('3DLED On Metal, Thermal Eng Off, Dungeon')
